...
if [[ -z "$INSTALL_ONLY" ]]; then
  ...
  cd "$REPO_DIR"
  PYTHONUNBUFFERED=1 service_loop "python main.py --dont-print-server --highvram --fast fp16_accumulation --preview-method none --port 7103" > $LOG_DIR/sd_comfy.log 2>&1 &
